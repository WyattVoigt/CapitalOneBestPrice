...
import pandas as pd
from recipes import find_recipes_with_ingredients
...
file_path_walmart = r'products_Walmart(kaggle).csv'
...
def nextpage():
    ...
    new_items = []
    for item in items_for_cheapest_store:
        print(cheapest_store)
        if cheapest_store == 'HEB':
            matches = df_heb[df_heb['title'].str.contains(item[0], case=False, na=False)]['item_type']
        elif cheapest_store == 'Target':
            matches = df_target[df_target['title'].str.contains(item[0], case=False, na=False)]['item_type']
        else:
            matches = df_walmart[df_walmart['Title'].str.contains(item[0], case=False, na=False)]['CATEGORY']

        print(matches)
        new_items.append((item, matches))
    ...
    return render_template(
         ...
         recipes=find_recipes_with_ingredients(new_items)[:10]
     )
